...
Friend Module ImportExport
    ...
    Public Class xlsRead
        ...
        Public Function OpenSource(ByVal myExcel As Excel.Application, ByVal filePath As String) As Boolean
            Try
                myWorkbook = myExcel.Workbooks.Open(filePath, ReadOnly:=True)
            Catch ex As Exception
                MsgBox(Err.Description, MsgBoxStyle.Exclamation, "Project X: xlsImport")
                Return False
            End Try
        ...
        Public Sub SoilDescription(ByRef manualBD As Boolean, ByRef manualFC As Boolean, ByRef manualPWP As Boolean, _
            ...
            If FileOps.WorksheetExists(curSheet, myWorkbook) Then
                ...
                If totalLayers > 0 Then
                    tempArray = Sheet.Range("A10").Resize(totalLayers, soilParameters).Value

                    For i As Integer = 1 To totalLayers
                        If IsNumeric(tempArray(i, 2)) Then LT(i) = tempArray(i, 2)
                        If IsNumeric(tempArray(i, 4)) Then clay(i) = tempArray(i, 4)
                        If IsNumeric(tempArray(i, 5)) Then sand(i) = tempArray(i, 5)
                        If IsNumeric(tempArray(i, 6)) Then IOM(i) = tempArray(i, 6)
                        If IsNumeric(tempArray(i, 7)) Then BD(i) = tempArray(i, 7)
                        If IsNumeric(tempArray(i, 8)) Then FC(i) = tempArray(i, 8)
                        If IsNumeric(tempArray(i, 9)) Then PWP(i) = tempArray(i, 9)
                        If IsNumeric(tempArray(i, 10)) Then NO3(i) = tempArray(i, 10)
                        If IsNumeric(tempArray(i, 11)) Then NH4(i) = tempArray(i, 11)
                    Next
                End If
            End If
        End Sub
        ...
        Private Function ReadFieldOps(ByVal curSheet As String, ByRef oplist(,) As Object, ByRef usingOps As Boolean) As Boolean
            ...
            If FileOps.WorksheetExists(curSheet, myWorkbook) Then
                ...
                If max > 0 Then
                    oplist = Sheet.Range("A6").Resize(max, UBound(oplist, Rank:=2)).Value
                End If
    ...
    Public Class xlsWrite
        ...
        Private Sub HeadersSoil(ByRef Sheet As Excel.Worksheet, ByVal curSheet As String)
            ...
            ReDim tempArray(2, soilParameters - 1)
            xlsRange = Sheet.Range("A7").Resize(3, soilParameters)
            Call SideSubs.HeaderSettings(xlsRange, "center")
